...
class KGProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type, data_dir, predict_what):
...
def main():
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                #print("logits, logits.shape = ", logits, logits.shape) #
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for test_triple in train_triples:
            ...
            # tmp_examples = processor._create_examples(head_corrupt_list, "test", args.data_dir, "head")

            # print("len(tmp_examples) = ", len(tmp_examples))
            ...
            #         #if n == 0:
            #             #print("(len(preds) == 0), batch_logits, shape = ", batch_logits, batch_logits.shape)
            ...
            #         #if n == 3:
            #             #print("batch_logits, shape = ", batch_logits, batch_logits.shape)
            #             #print("preds[0], shape = ", preds[0], preds[0].shape)
            ...
            # thor, argsort1 = torch.sort(rel_values, descending=True)
            # #print(max_values)

            # #print("thor = ", thor)

            # #print("argsort1, shape = ", argsort1, argsort1.shape)

            # argsort1 = argsort1.cpu().numpy() #tensor to numpy
            ...
            tmp_examples = processor._create_examples(tail_corrupt_list, "test", args.data_dir, "tail")
            ...
            eval_sampler = SequentialSampler(eval_data)
            # eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size)
            train_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size)
            model.eval()
            preds = []
            ...
            rel_values = preds[:, all_label_ids[0]]
            rel_values = torch.tensor(rel_values)
            value, argsort1 = torch.sort(rel_values, descending=True)
        ...
        logger.info('sum(predicted_class): {0}'.format(sum(predicted_class)))
        logger.info('len(predicted_class): {0}'.format(len(predicted_class)))
        logger.info('Accuracy: {0}'.format(sum(predicted_class)/len(predicted_class)))
