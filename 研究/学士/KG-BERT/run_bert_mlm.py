...
class KGProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type, data_dir):
        # """Creates examples for the training and dev sets.""" # relation の予測
        # # entity to text
        # ent2text = {}
        ...
        # """Creates examples for the training and dev sets.""" # tail の予測分類
        # # entity to text
        # ent2text = {}
        # with open(os.path.join(data_dir, "entity2text.txt"), 'r') as f:
        #     ent_lines = f.readlines()
        ...
        # for (i, line) in enumerate(lines):
        #     guid = "%s-%s" % (set_type, i)
        #     text_a = ent2text[line[0]]  # tail predict
        #     text_b = rel2text[line[1]]  
        #     label = line[2]
        #     # text_a = rel2text[line[1]]  # head predict
        #     # text_b = ent2text[line[2]]
        #     # label = line[0]
        #     self.labels.add(label)
        #     examples.append(
        ...
        """Creates examples for the training and dev sets.""" # MLM
        # entity to text
        ent2text = {}
        with open(os.path.join(data_dir, "entity2text.txt"), 'r') as f:
            ent_lines = f.readlines()
        ...
        for (i, line) in enumerate(lines):
            ...
            if set_type == "dev" or set_type == "test":
                ...
                #if predict_what == "tail":
                #    text_a = text_a[:text_a.find(',')]
                #    if i == 0:
                #        print("text_a = ", text_a)
                ...
                #if predict_what == "head":
                #    text_c = text_c[:text_c.find(',')]
                #    if i == 0:
                #        print("text_c = ", text_c)
                ...
                text_c = tail_ent_text 
                examples.append(
                    InputExample(guid=guid, text_a=text_a, text_b=text_b, text_c = text_c, label=line[2]))
...
def main():
    ...
    label_list = processor.get_entities(args.data_dir)
    ...
    if args.do_train:
        ...
        loss_file = os.path.join(args.output_dir, "loss.pickle")
        with open(loss_file, 'wb') as p:
            pickle.dump(tr_loss_list, p)
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        exist_hits_n = [[] for i in range(500)]
        exist_head_hits_n = []
        exist_relation_hits_n = []
        ...
        for i, pred in enumerate(preds):
            ...
            for tmp_label_id in argsort1[:rank]:
                ...
                tmp_triple = [test_triple[0], test_triple[1], tmp_label]  # tail predict
                # tmp_triple = [tmp_label, test_triple[1], test_triple[2]]  # head predict
            ...
            count_exist_triple = 0
            count_exist_head_triple = 0
            count_exist_relation_triple = 0
            exist_triple = []
            for tmp_label_id in argsort1:  # head, relation (推定するエンティティ以外) が同じトリプルの数
                tmp_label = label_list[tmp_label_id]
                tmp_triple = [test_triple[0], test_triple[1], tmp_label]  # tail predict
                # tmp_triple = [tmp_label, test_triple[1], test_triple[2]]  # head predict
                tmp_triple_str = '\t'.join(tmp_triple)
                if tmp_triple_str in all_triples_str_set:
                    exist_triple.append(tmp_triple_str)
                    count_exist_triple += 1
                # if tmp_label in all_triples[:,2]:
                #     if test_triple[0] in all_triples[:,0]:
                #         count_exist_head_triple += 1
                #     if test_triple[1] in all_triples[:,1]:
                #         count_exist_relation_triple += 1
            # print("count_exist_triple, exist_triple = ", count_exist_triple, exist_triple)

            exist_rank = 0
            for tmp_label_id in argsort1[:count_exist_triple]:  # exist_hits の計算
                tmp_label = label_list[tmp_label_id]
                tmp_triple = [test_triple[0], test_triple[1], tmp_label]  # tail predict
                # tmp_triple = [tmp_label, test_triple[1], test_triple[2]]  # head predict
                tmp_triple_str = '\t'.join(tmp_triple)
                if tmp_triple_str in all_triples_str_set:
                    exist_rank += 1
            exist_hits_n[count_exist_triple].append(exist_rank/count_exist_triple)
            print("exist_rank, exist_hits_n = ", exist_rank, exist_hits_n[count_exist_triple])

            # exist_head_rank = 0
            # for tmp_label_id in argsort1[:count_exist_head_triple]:  # exist_head_hits の計算
            #     tmp_label = label_list[tmp_label_id]
            #     if tmp_label in all_triples[:,2]:
            #         if test_triple[0] in all_triples[:,0]:
            #             exist_head_rank += 1
            # exist_head_hits_n.append(exist_head_rank/count_exist_head_triple)

            # exist_relation_rank = 0
            # for tmp_label_id in argsort1[:count_exist_relation_triple]:  # exist_relation_hits の計算
            #     tmp_label = label_list[tmp_label_id]
            #     if tmp_label in all_triples[:,2]:
            #         if test_triple[1] in all_triples[:,1]:
            #             exist_relation_rank += 1
            # exist_relation_hits_n.append(exist_relation_rank/count_exist_relation_triple)
        ...
        new_exist_hits_n = exist_hits_n
        # new_exist_hits_n = [round(exist_hits_n[n], 3) for n in range(len(exist_hits_n))]
        # new_exist_head_hits_n = [round(exist_head_hits_n[n], 3) for n in range(len(exist_head_hits_n))]
        # new_exist_relation_hits_n = [round(exist_relation_hits_n[n], 3) for n in range(len(exist_relation_hits_n))]

        new_exist_hits = torch.tensor(new_exist_hits_n)
        # new_exist_hits, _ = torch.sort(new_exist_hits, descending=True)
        new_exist_hits = new_exist_hits.cpu().numpy()

        # print("Exist Hits@n: ", new_exist_hits_n[:10])
        for i in range(500):
            if len(exist_hits_n[i]) > 0:
                print("Mean Exist Hits@n: ", i, np.mean(exist_hits_n[i]))
        
        exist_hits_file = os.path.join(args.output_dir, "exist_hits.pickle")
        with open(exist_hits_file, 'wb') as p:
            pickle.dump(exist_hits_n, p)

        # new_exist_hits = torch.tensor(new_exist_head_hits_n)
        # new_exist_hits, _ = torch.sort(new_exist_hits, descending=True)
        # new_exist_hits = new_exist_hits.cpu().numpy()

        # print("Exist head Hits@n: ", new_exist_hits_n[:10])
        # print("Mean Exist head Hits@n: ", np.mean(exist_hits_n))

        # new_exist_hits = torch.tensor(new_exist_relation_hits_n)
        # new_exist_hits, _ = torch.sort(new_exist_hits, descending=True)
        # new_exist_hits = new_exist_hits.cpu().numpy()

        # print("Exist relation Hits@n: ", new_exist_hits_n[:10])
        # print("Mean Exist relation Hits@n: ", np.mean(exist_hits_n))

        print("Raw mean rank: ", np.mean(ranks))
        print("Filtered mean rank: ", np.mean(filter_ranks))
        print("Raw mean reciprocal rank: {0}".format(np.mean(1./np.array(ranks))))
        print("Filtered mean reciprocal rank: {0}".format(np.mean(1./np.array(filter_ranks))))
        for i in [0,2,9]:
        ...
        print("Tail prediction hits@1, raw...")
