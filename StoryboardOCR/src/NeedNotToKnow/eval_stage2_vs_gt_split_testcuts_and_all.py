# eval_stage2_vs_gt_split_testcuts_and_all.py
# ------------------------------------------------------------
# Evaluate Stage2 (CLM correction) outputs against annotation_table.csv GT.
#
# We compute metrics for:
#  (A) only cuts that were used as "test_cut_ids" in training split_by_cut.json
#  (B) all cuts (all GT rows in annotation_table.csv)
#
# Stage2 source:
#  outputs_stage2_clm/episode01/cuts_split/cutXXXX.stage2.split.{nojp|jp}.json
#
# We compare:
#  - Stage1 raw (from stage2 split cell["raw_text"])  vs GT
#  - Stage2 corrected (cell["corrected_text"])        vs GT
#
# Metrics:
#  - CER (uncapped):  edit_distance / len(ref)
#  - CER_micro:       sum(edit_distance) / sum(len(ref))
#  - ExactMatch:      normalized strings match (space-insensitive + neutral removed)
#  - CharAccuracy:    1 - CER  (also provide clipped version to [0,1])
#
# Output:
#  - JSON summary under CFG["out_dir"]
#  - Optional JSONL per-sample dump for debugging
# ------------------------------------------------------------
...
CFG = {
    ...
    #  - "nojp": (term/char/dialogue only)
    #  - "jp":   (plus Japanese polish)
    #  - "both": evaluate both and write two result JSONs
    ...
    # Output
    "out_dir": "outputs_stage2_clm/episode01/eval_stage2_vs_gt",
...
class Agg:
    ...
    def add(self, hyp: str, ref: str, neutral_symbols: List[str]):
        ...
        # clipped accuracy (useful if CER>1 due to many insertions)
...
def build_stage2_split_path_map(stage2_split_dir: str) -> Dict[str, Dict[int, str]]:
    ...
    for fn in os.listdir(stage2_split_dir):
        ...
        cut_i = int(m.group(1))
        variant = m.group(2)
        paths[variant][cut_i] = os.path.join(stage2_split_dir, fn)
    return paths
...
def evaluate_subset(
    *,
    ...
    """
    # caches
    cut_cache: Dict[int, Dict[Tuple[int, int, str], Dict[str, str]]] = {}
    ...
    try:
        for sp in samples:
            ...
            if cut not in cut_cache:
                cut_cache[cut] = load_stage2_split_cells(s2_path)
            ...
            hyp_s1 = cell.get("raw_text", "")
            hyp_s2 = cell.get("corrected_text", "")
            ref = sp.ref
        ...
        return {
            ...
            "counts": {
                ...
                "missing_cell_in_stage2": missing_cell,
...
def main_one_variant(variant: str):
    ...
    # Evaluate
    out_jsonl_all = os.path.join(CFG["out_dir"], f"samples_all.{variant}.jsonl") if CFG["write_samples_jsonl"] else None
    out_jsonl_test = os.path.join(CFG["out_dir"], f"samples_testcuts.{variant}.jsonl") if CFG["write_samples_jsonl"] else None
    ...
    out = {
        "meta": {
            ...
            "note": "Stage1 raw is read from stage2 split cell['raw_text']",
        },
        "train_split": {
    ...
    out_path = os.path.join(CFG["out_dir"], f"eval_stage2_vs_gt_split.{variant}.json")
    ...
    def _p(label: str, obj: Dict[str, Any]):
        ...
        print(f"\n=== {label} ({variant}) ===")
        print("samples_evaluated:", obj["counts"]["samples_evaluated"])
