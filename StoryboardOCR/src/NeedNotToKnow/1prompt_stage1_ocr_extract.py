...
CFG = {
    # Input
    "episode_id": "episode01",  # "episode01"
    ...
    # 例: "data/storyboard_symbol_lexicon.json"
...
def load_storyboard_symbol_words(path: str) -> List[str]:
    ...
    の word を全て集めて重複排除し、短い順ではなく「長い順」優先で返す。
    """
    ...
    # 長い表記を先に（O.L など短いのは最後でもOKだが、ここは一旦長い順）
...
def build_ocr_system_prompt(symbol_words: List[str], lexicon_characters: List[str],) -> str:
    """
    意味は入れず、用語リスト（羅列）のみをプロンプトに含める。
    """
    glossary = ""
    if symbol_words:
        # できるだけシンプルに羅列（改行区切り）
        glossary = "=== STORYBOARD SYMBOL GLOSSARY (terms only) ===\n" + "\n".join(symbol_words) + "\n\n"
    ...
    prompt = (
        ...
        "Handwritten text may include arrows and symbols, and may be less neat.\n\n"

        "Task: transcribe ALL visible text as faithfully as possible.\n"
        "Rules:\n"
        "- Keep original line breaks.\n"
        "- Preserve symbols exactly (e.g., arrows like → ← ↑ ↓, brackets, punctuation).\n"
        ...
        "Important:\n"
        "- The text may include production terms/symbols and character names; prefer transcribing them exactly.\n"
...
@torch.no_grad()
def ocr_one_image(model, processor, image: Image.Image, ocr_system_prompt: str) -> Dict[str, Any]:
    """
...
def main():
    ...
    # ★ NEW: load symbol words and build OCR prompt once
    symbol_words = load_storyboard_symbol_words(CFG["symbol_lexicon_path"])
    lexicon_characters = load_lexicon_characters(os.path.join("outputs", CFG["script_phase"], CFG["episode_id"], CFG["lexicon_path"]))
    ocr_system_prompt = build_ocr_system_prompt(symbol_words=symbol_words, lexicon_characters=lexicon_characters,)
    ...
    def ocr_path(path: str) -> Dict[str, Any]:
        ...
        img = load_image(path)
        res = ocr_one_image(model, processor, img, ocr_system_prompt)
        ocr_cache[path] = res
        return res
    ...
    index: Dict[str, Any] = {
        ...
        "meta": {
            ...
            "symbol_word_count": len(symbol_words),
        },
    }
